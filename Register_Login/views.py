...
from Register_Login.serializers import LoginSerializer, UserSerializer,LocationUpdateSerializer
...
# Get Delivery Guy
class get_delivery_guys(APIView):
    def get(self,request):
        all = Profile.objects.filter(is_active = True,groups=2,)
        serializer = UserSerializer(all,many = True)
        return JsonResponse({"Names": serializer.data}, safe=False)
